...
export default function AlbumsPage() {
  ...
  const [albumImages, setAlbumImages] = useState<Record<string, Image[]>>({})
  ...
  const [currentDownloadImage, setCurrentDownloadImage] = useState<string | null>(null)
  ...
  const fetchAlbums = useCallback(async () => {
    ...
    try {
      ...
      const validAlbums = data
        .filter((album: any) => album && (album.title || album._id))
  ...
  // Fetch album images
  const fetchAlbumImages = useCallback(async (albumTitle: string, retries = 3) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        const processedData = data.slice(0, 20).map((image: any) => {
  ...
  const downloadImage = useCallback(async (imageUrl: string, filename: string, format: string) => {
    try {
