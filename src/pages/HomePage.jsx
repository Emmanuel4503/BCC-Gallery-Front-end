...
function HomePage() {
  ...
  const [carouselImageLoading, setCarouselImageLoading] = useState({});
  ...
  const fetchCarouselImages = async () => {
    try {
      ...
      const initialLoadingState = data.reduce((acc, image) => {
        acc[image._id || image.imageUrl] = true;
        return acc;
      }, {});
      setCarouselImageLoading(initialLoadingState);
    ...
    try {
      ...
      if (format === "heic") {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch HEIC image: ${response.status} ${response.statusText} - ${text.slice(0, 100)}`
          );
        }
  ...
  const handleMultipleDownload = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (failCount > 0) {
          alert(
            `Download completed with issues. ${successCount} images downloaded successfully, ${failCount} failed.`
          );
        } else {
  ...
  const scrollToTop = () => {
    ...
    if (gallery) {
      window.scrollTo({
        top: gallery.offsetTop - headerHeight,
        behavior: "smooth",
      });
    } else {
      window.scrollTo({
  ...
  // Timeout for spinners
  useEffect(() => {
    const timeoutIds = Object.keys(carouselImageLoading)
      .concat(Object.keys(galleryImageLoading))
      .map((id) => {
        if (carouselImageLoading[id] || galleryImageLoading[id]) {
          return setTimeout(() => {
            setCarouselImageLoading((prev) => ({ ...prev, [id]: false }));
            setGalleryImageLoading((prev) => ({ ...prev, [id]: false }));
            setFailedImages((prev) => new Set([...prev, id]));
            addNotification("Image loading timed out");
          }, 10000);
        }
        return null;
      })
  ...
  }, [carouselImageLoading, galleryImageLoading]);
      ...
      <div className="notification-container">
        {currentNotification && (
          <div className="notification">
            ...
            <button className="notification-close" onClick={removeCurrentNotification}>
                  ...
                  {downloadType === "multiple" && (
                    <div className="download-warning">
                      <p className="warning-text">
                        <strong>Note:</strong> To download multiple images, please ensure your browser allows
                        multiple download from this site. You may see a prompt in your browser’s address bar to
                        allow multiple downloads. Click "Allow" to proceed.
                      </p>
                    </div>
                  )}
                ...
                <div className="download-progress-section">
                  <div className="progress-info">
                    ...
                    <p className="progress-subtitle">
                      {downloadType === "multiple" &&
                        `${Math.ceil((downloadProgress / 100) * selectedImages.length)} of ${
                  ...
                  <div className="progress-status">
                    ...
                    <span>
                      Please wait while we prepare your {downloadType === "single" ? "image" : "images"}...
                    </span>
                  </div>
                </div>
              )}
      ...
      <div className="hero-section">
        ...
        <div className="hero-content">
          ...
          <p className="welcome-description">
            Discover the <span className="text-highlight">beautiful moments</span> and memories captured in
            the church community. <span className="text-bold">Browse through the collection of</span>{" "}
        ...
        <div className="carousel-container">
          {isLoadingCarousel ? (
            ...
            <div className="error-container">
              ...
              <button onClick={fetchCarouselImages} className="retry-btn">
                Retry
              </button>
            </div>
            ...
            <div className="carousel">
              {carouselImages.map((image, index) => {
                ...
                const isImageLoading = carouselImageLoading[image._id || image.imageUrl];
                const imageSrc = failedImages.has(image._id || image.imageUrl)
                  ? "/placeholder.svg"
                  : image.imageUrl;
                return (
                  <div key={image._id || index} className={slideClass}>
                    ...
                    <img
                      ...
                      onLoad={() => {
                        console.log(`Carousel image ${index + 1} loaded: ${imageSrc}`);
                        setCarouselImageLoading((prev) => ({
                          ...prev,
                          [image._id || image.imageUrl]: false,
                        }));
                      }}
                      onError={() => {
                        console.error(`Failed to load carousel image ${index + 1}: ${imageSrc}`);
                        setCarouselImageLoading((prev) => ({
                          ...prev,
                          [image._id || image.imageUrl]: false,
                        }));
                        setFailedImages((prev) => new Set([...prev, image._id || image.imageUrl]));
                        addNotification(`Failed to load carousel image ${index + 1}`);
                      }}
                    />
                  ...
                  <div key={image._id || index} className="image-card">
                    <div className="image-container">
                      ...
                      <img
                        ...
                        onLoad={() => {
                          console.log(`Gallery image ${index + 1} loaded: ${imageSrc}`);
                        ...
                        onError={() => {
                          console.error(`Failed to load gallery image ${index + 1}: ${imageSrc}`);
                          setGalleryImageLoading((prev) => ({
                            ...prev,
                            [image._id || image.imageUrl]: false,
                          }));
                    ...
                    <div className="reaction-section">
                      <button
                        className={`reaction-btn ${
                          isUserReacted(image._id, "partyPopper") ? "reaction-active" : ""
                        }`}
                      ...
                      <button
                        className={`reaction-btn ${
                          isUserReacted(image._id, "thumbsUp") ? "reaction-active" : ""
                        }`}
                      ...
                      <button
                        className={`reaction-btn ${
                          isUserReacted(image._id, "redHeart") ? "reaction-active" : ""
                        }`}
      ...
      {fullscreenImage && (
        <div className="fullscreen-modal" onClick={closeFullscreen}>
          ...
          <div className="fullscreen-content" onClick={(e) => e.stopPropagation()}>
            <img src={fullscreenImage || "/placeholder.svg"} alt="Fullscreen view" className="fullscreen-image" />
          </div>
        </div>
      )}
    </div>
